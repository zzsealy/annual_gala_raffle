...
export default function ConfigPage() {
  ...
  const fetchQueues = async () => {
    try {
      const res: any[] = await ky.get(`${process.env.NEXT_PUBLIC_API_URL}/api/raffle_queue`).json();
      setQueues(res);
    } catch (e) {
      console.error(e);
    }
  ...
  const handleReset = async () => {
    if (!window.confirm("❗ 警告：这将清空所有已有中奖记录，重置所有已中奖人的状态，并将抽奖队列恢复为初始状态！\n\n确定要重置吗？")) return;
    
    try {
      if (isSubmitting) return;
      setIsSubmitting(true);
      await ky.post(`${process.env.NEXT_PUBLIC_API_URL}/api/raffle_queue/reset`);
      alert("重置成功！所有数据已被清空。");
      fetchQueues();
    } catch(e) {
      console.error(e);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center relative">
      ...
      <div className="bg-white p-8 rounded-xl shadow-2xl w-[450px] border border-gray-200 flex flex-col h-[700px]">
        <h1 className="text-2xl font-black text-gray-900 mb-6 border-b-2 border-gray-100 pb-3">📜 实时抽奖队列</h1>
            ...
            {queues.map((q, index) => (
              ...
              >
                <div className="flex flex-col gap-1">
                  <div className="text-xl font-bold text-gray-800">{q.desc}</div>
                  <div className="text-sm text-gray-500 font-medium">抽取人数: <span className="text-red-600 font-bold">{q.raffleQueuePersonNum}</span> 人</div>
                </div>
        ...
        <div className="mt-6 pt-6 border-t font-semibold border-gray-100 text-center">
          ...
          <p className="text-xs text-gray-500 mt-3 font-normal">点击将清空记录、还原中奖名单并重置队列</p>
        </div>
      </div>
